...
void LogBytes(const std::vector<CharType>& data, std::string* out)
{
    ...
    if (data.size() > kMaxBytesToLog) {
        snprintf(buf, sizeof(buf), " and %u more bytes",
...
void WriteValue(Message* m, const base::Value* value, int recursion)
{
    ...
    case base::Value::TYPE_BINARY: {
        ...
        m->WriteData(static_cast<const char*>(binary->GetBuffer()), static_cast<int>(binary->GetSize()));
        break;
    }
               ...
               int recursion)
{
    ...
    case base::Value::TYPE_DICTIONARY: {
        //DictionaryValue> val(new base::DictionaryValue());
    ...
    case base::Value::TYPE_LIST: {
        ...
        base::ListValue* val = new base::ListValue();
...
template <class P>
void param_to_string(const P& p, std::string* l)
{
    std::stringstream ss;
    ss << p;
    l->append(ss.str());
...
{
    std::string json;
    base::JSONWriter::Write(&p, true, &json);
    l->append(json);
}
