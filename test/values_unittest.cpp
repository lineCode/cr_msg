...
TEST(ValuesTest, List) {
    boost::shared_ptr<ListValue> mixed_list(new ListValue());
...
TEST(ValuesTest, BinaryValue) {
    ...
    boost::shared_ptr<BinaryValue> binary(new BinaryValue());
    ASSERT_EQ(NULL, binary->GetBuffer());
    ASSERT_EQ(0U, binary->GetSize());

    // Test the common case of a non-empty buffer
    //char* buffer = new char[15];
    //binary.reset(new BinaryValue(buffer, 15));
...
TEST(ValuesTest, StringValue) {
    ...
    boost::shared_ptr<Value> narrow_value(new StringValue("narrow"));
...
TEST(ValuesTest, ListRemoval) {
    bool deletion_flag = true;
    boost::shared_ptr<Value> removed_item;
    Value* removed_ptr = NULL;

    {
        ...
        EXPECT_FALSE(list.Remove(1, &removed_ptr));
        EXPECT_FALSE(removed_ptr);
        EXPECT_TRUE(list.Remove(0, &removed_ptr));
        removed_item.reset(removed_ptr);
        removed_ptr = NULL;
...
TEST(ValuesTest, DeepCopy) {
    ...
    boost::shared_ptr<DictionaryValue> copy_dict(original_dict.DeepCopy());
...
TEST(ValuesTest, Equals) {
    ...
    boost::shared_ptr<DictionaryValue> copy;
...
TEST(ValuesTest, StaticEquals) {
    boost::shared_ptr<Value> null1(Value::CreateNullValue());
    boost::shared_ptr<Value> null2(Value::CreateNullValue());
    EXPECT_TRUE(Value::Equals(null1.get(), null2.get()));
    EXPECT_TRUE(Value::Equals(NULL, NULL));

    boost::shared_ptr<Value> i42(new FundamentalValue(42));
    boost::shared_ptr<Value> j42(new FundamentalValue(42));
    boost::shared_ptr<Value> i17(new FundamentalValue(17));
...
TEST(ValuesTest, DeepCopyCovariantReturnTypes) {
    ...
    boost::shared_ptr<Value> copy_dict_value(original_dict_value->DeepCopy());
    boost::shared_ptr<Value> copy_bool_value(original_bool_value->DeepCopy());
    boost::shared_ptr<Value> copy_int_value(original_int_value->DeepCopy());
    boost::shared_ptr<Value> copy_double_value(original_double_value->DeepCopy());
    boost::shared_ptr<Value> copy_string_value(original_string_value->DeepCopy());
    boost::shared_ptr<Value> copy_string16_value(original_string16_value->DeepCopy());
    boost::shared_ptr<Value> copy_list_value(original_list_value->DeepCopy());
...
TEST(ValuesTest, RemoveEmptyChildren) {
    boost::shared_ptr<DictionaryValue> root(new DictionaryValue);
...
TEST(ValuesTest, MergeDictionary) {
    boost::shared_ptr<DictionaryValue> base(new DictionaryValue);
    ...
    boost::shared_ptr<DictionaryValue> merge(new DictionaryValue);
...
TEST(ValuesTest, MergeDictionaryDeepCopy) {
    ...
    boost::shared_ptr<DictionaryValue> merged(new DictionaryValue);
